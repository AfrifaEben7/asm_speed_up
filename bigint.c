...
void bigint_shift_right_void(bigint l, int shamt);
...
{
  bigint r = bigint_from_int(0); 
  bigint zero = bigint_from_int(0);
  bigint num = bigint_copy(b); 
  bigint tmp, resplusbit, bit;
...
/* Arithmetic shift right the given about.  This will shift by
   ...
   shift. it returns void*/

void bigint_shift_right_void(bigint l, int chunks) {
  schunk tmpc;
  int extra = chunks % BITSPERCHUNK;
  chunks = chunks / BITSPERCHUNK;
  l ->size -= chunks;
  if (extra) {
...
/*This is a new bigint_trim function. RETURNS VOID.*/
...
void bigint_trim_void(bigint b) {
  ...
  if (i > 0) 
  {
    ...
    { 
      // we have a leading block that is all 1
      do
	      i-=1; // search for first block that is not all 1
      while ((i>0) && (b->blks[i]==CHUNKMASK));
  
      if (!(b->blks[i] & ((bigchunk)1<<(BITSPERCHUNK-1))))
	      i+=1;  // if msb of current block is 0, then we went too far
    }
  }  // i is now the number of blocks to copy  
